...
\section{Modell B: Kurzzeit-Modell}
\vspace{-0.5cm}
\noindent\textit{\small Verfasst von Kilian}
\vspace{0.5cm}


\subsection{Vorbemerkung: Methodische Korrektur nach der Abschlusspräsentation (14.01.2026, Schneefernerhaus)}
Nach der Präsentation wurde ein Fehler in der Trainingsdatenzuweisung identifiziert.
Einzelne Referenzpunkte waren fälschlich als positive Ereignisse gelabelt, ohne dokumentierte Erdrutsche zu repräsentieren.
Da überwachte Lernverfahren konsistente Ground-Truth Daten erfordern, ist eine solche Fehlklassifikation methodisch unzulässig und kann Entscheidungsgrenzen verzerren \autocite{breimanRandomForests2001}.
Die Validierung der präsentierten Version basierte ausschließlich auf korrekten Testlabels.
Der Fehler betraf die Zusammensetzung des Trainingsdatensatzes.
Nach erneuter Prüfung des Ereigniskatasters wurden alle Trainingslabels aus verifizierten Ereignissen abgeleitet.
Das Modell wurde neu trainiert.
Die erneute Modellierung zeigte vergleichbare Leistungskennzahlen und bestätigt die strukturelle Stabilität des Ansatzes.
Gleichwohl ist ein Training mit fehlerhaften Positivlabels wissenschaftlich nicht akzeptabel.
Die transparente Korrektur ist daher zentral für die methodische Validität des finalen Modells.
Im Zuge der Überarbeitung erfolgte zudem eine Modelloptimierung mit angepassten Parametern, überprüfter Klassenbalance und datenbasierter Schwellenwertkalibrierung.
...
\subsubsection{Hangneigung, Profil- und Plankrümmung}
...
Die Plan- und Profilkrümmung werden nach \textcite{zevenbergenQuantitativeAnalysisLand1987} über eine lokale quadratische Oberflächenapproximation in einem $3 \times 3$-Rasterfenster berechnet.
...
kennzeichnet eine konvexe (upwardly convex) Form mit tendenzieller Abflussverzögerung, während
\[
    \mathrm{ProC} > 0
\]

eine konkave (upwardly concave) Form mit Abflussbeschleunigung beschreibt.
...
\[
    \mathrm{PlnC} < 0
\]
steht für lateral konkave, konvergente Formen (Rinnen) mit lateraler Wasserzusammenführung,

\[
    \mathrm{PlnC} > 0
\]
für lateral konvexe, divergente Formen (Rücken) mit Wasserverteilung \autocite{CurvatureFunctionArcGIS} .
...
\subsubsection{Modifizierter Topographic Wetness Index (TWI)}
...
\subsubsection{ERA5-Reanalyse und orographische Korrekturen}

ERA5 Singel Levels liegen nativ in einer räumlichen Auflösung von etwa 0,25° (28 km) vor \autocite{ERA5HourlyData}. Um die Skaleninkonsistenz zwischen einer homogenen ERA5 0,25° Zelle und einer 500-m Raster Zelle physikalisch zu reduzieren, werden orographische Korrekturen implementiert. Dazu wird eine ERA5-Referenzorographie abgeleitet und die Höhendifferenz \(\Delta h\) zwischen 500-m Zelle und ERA5-Referenzhöhe gebildet.

Die 2-m-Temperatur wird zunächst von Kelvin nach Celsius transformiert

\[T_{^\circ\mathrm{C}} = T_{\mathrm{K}} - 273{,}15\]

Anschließend erfolgt eine höhenabhängige Anpassung über den klimatologischen Standardgradienten der Environmental Lapse Rate
...
entsprechend einer orographischen Niederschlagszunahme von +8 \% pro 100 Höhenmeter. Dieser Wert liegt innerhalb des in der alpinen Niederschlagsklimatologie dokumentierten Bereichs von etwa 5–10 \% pro 100 m \autocite{freiPrecipitationClimatologyAlps1998, sevrukRegionalDependencyPrecipitationAltitude1997}
...
einer Rutschung. Die binäre Klassifikation erfolgt über einen Schwellenwert \( \tau \).
Die Verwendung eines festen Schwellenwerts von \( \tau = 0.50 \)
...
Precision und Recall dar.
Mit \( \beta = 0.6 \) wird Precision
stärker gewichtet als Recall, wodurch Fehlalarme stärker penalisiert
werden als einzelne verpasste Ereignisse \autocite{pedregosaScikitlearnMachineLearning2011}.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{img/landslideharadmodel2020.png}
    \caption{Berechnete Risikokarte für Erdrutsche des 08.05.2021, Südtirol}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{img/landslideharzadmodel202012.png}
...
Der erste Betrachtungstag weist großflächig erhöhte Wahrscheinlichkeiten auf. Die an diesem Tag dokumentierten Erdrutschereignisse korrelieren räumlich mit den vom Modell ausgewiesenen Hochrisikobereichen. 94,4 \% dokumentierter Ereignisse wurden am 08.05.2021 korrekt erfasst.
Die erhöhte Disposition verteilt sich über mehrere zusammenhängende Zonen und spiegelt eine ausgeprägte hydrometeorologische Vorbelastung wieder.
